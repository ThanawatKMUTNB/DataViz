...
class csvManager:
    ...
    def getDimension(self):
        Dimen = []
        for head in self.df.columns:
            if (self.df.dtypes[head] == 'object' or head == 'Row ID' or head == 'Postal Code'):
                Dimen.append(head)
        return Dimen
    ...
    def loadPandas(self) :
        ...
        if "Dimen" in metadata :
            Dimension.append(metadata["Dimen"])
            print(Dimension)
        elif "Meas" in metadata :
            Measurment.append(metadata["Meas"])
            print(Measurment)
...
#ex.getValueDimension(["Segment","Sales","Profit"])
ex.loadPandas()